...
layui.define(['laytpl', 'ulaypage', 'layer', 'uform', 'util'], function(exports){
  "use strict";
  
  var $ = layui.$
  ,laytpl = layui.laytpl
  ,laypage = layui.ulaypage
  ,layer = layui.layer
  ,form = layui.uform
  ,util = layui.util
  ,hint = layui.hint()
  ...
  //字符常量
  ,MOD_NAME = 'utable', ELEM = '.layui-table', THIS = 'layui-this', SHOW = 'layui-show', HIDE = 'layui-hide', DISABLED = 'layui-disabled', NONE = 'layui-none'
          ...
          ,'<th data-field="{{ item2.field||i2 }}" data-key="{{d.index}}-{{i1}}-{{i2}}" {{# if( item2.parentKey){ }}data-parentkey="{{ item2.parentKey }}"{{# } }} {{# if(item2.minWidth){ }}data-minwidth="{{item2.minWidth}}"{{# } }} '+ rowCols +' {{# if(item2.unresize || item2.colGroup){ }}data-unresize="true"{{# } }} class="{{# if(item2.hide){ }}layui-hide{{# } }}{{# if(isSort){ }} layui-unselect{{# } }}{{# if(!item2.field){ }} layui-table-col-special{{# } }}">'
            ...
            ,'" {{#if(item2.align){}}align="{{item2.align}}"{{#}}}>'
              ...
              ,'{{# } else { }}'
                ...
                ,'{{# if(isSort){ }}'
                  ,'<span class="layui-table-sort layui-inline"><i class="layui-edge layui-table-sort-asc" title="{{ appLang.trans(\'升序\') }}"></i><i class="layui-edge layui-table-sort-desc" title="{{ appLang.trans(\'降序\') }}"></i></span>'
                ,'{{# } }}'
              ,'{{# } }}'
            ,'</div>'
          ,'</th>'
  ...
  Class.prototype.renderToolbar = function(){
    ...
    var layout = {
      filter: {
        title:  appLang.trans('筛选列')
        ,layEvent: 'LAYTABLE_COLS'
        ,icon: 'layui-icon-cols'
      }
      ,exports: {
        title:  appLang.trans('导出')
        ,layEvent: 'LAYTABLE_EXPORT'
        ,icon: 'layui-icon-export'
      }
      ,print: {
        title:  appLang.trans('打印')
  ...
  Class.prototype.pullData = function(curr){
    ...
    if(options.url){ //Ajax请求
      ...
      $.ajax({
        ...
        ,error: function(e, m){
          that.errorView( appLang.trans('数据接口请求异常')+":"+ m);
        ...
        case 'LAYTABLE_EXPORT': //导出
          if(device.ie){
            ...
            openPanel({
              list: function(){
                return [
                  '<li data-type="csv">'+appLang.trans('导出到 Csv 文件')+'</li>'
                  ,'<li data-type="xls">'+appLang.trans('导出到 Excel 文件')+'</li>'
                ].join('')
              }()
