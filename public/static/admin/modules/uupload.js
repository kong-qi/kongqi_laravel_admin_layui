...
layui.define('layer' , function(exports){
  ...
  //字符常量
  ,MOD_NAME = 'uupload', ELEM = '.layui-upload', THIS = 'layui-this', SHOW = 'layui-show', HIDE = 'layui-hide', DISABLED = 'layui-disabled'
  ...
  Class.prototype.upload = function(files, type){
    ...
    ,ajaxSend = function(){
      ...
      layui.each(items, function(index, file){
        ...
        var opts = {
          ...
          ,error: function(){
            aborted++;
            that.msg(appLang.trans('请求上传接口出现异常'));
            error(index);
            allDone();
          }
        };
    ...
    ,iframeSend = function(){
      ...
      Class.timer = setInterval(function() {
        ...
        try {
          ...
          that.msg(appLang.trans('获取上传后的响应信息出现异常'));
          clearInterval(Class.timer);
          error();
        }
        if(res){
    ...
    ,done = function(index, res){
      ...
      if(typeof res !== 'object'){
        try {
          ...
          res = {};
          return that.msg(appLang.trans('请对上传接口返回有效JSON'));
        }
      }
    ...
    switch(options.accept){
      ...
      default: //图片文件
        ...
        if(check){
          that.msg(appLang.trans('选择的图片中包含不支持的格式'));
          return elemFile.value = '';
        }
      break;
    }
    ...
    if(options.number && that.fileLength > options.number){
      return that.msg(appLang.trans('同时最多只能上传的数量为')+'：'+ options.number);
    }
    ...
    if(options.size > 0 && !(device.ie && device.ie < 10)){
      ...
      if(limitSize) return that.msg(appLang.trans('文件不能超过')+ limitSize);
    }
    send();
  };
  ...
  Class.prototype.events = function(){
    ...
    ,setChooseText = function(files, filename){
      ...
      ,value = files.length > 1 
        ? files.length + appLang.trans('个文件')
