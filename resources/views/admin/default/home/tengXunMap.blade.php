...
<head>
    ...
    <script charset="utf-8" src="https://map.qq.com/api/js?v=2.exp&key=PLIBZ-P5PWU-RWBVI-2CGW6-LJ6IT-UZFVR"></script>
    <style type="text/css">
        * {
            margin: 0px;
            padding: 0px;
...
<body>
<div style="position:relative; min-width: 930px">
...
<script type="text/javascript">
    ...
    /*这里需要替换为你们自己的秘钥，不然的话，可能会有额度限制*/
    /*将PLIBZ-P5PWU-RWBVI-2CGW6-LJ6IT-UZFVR替换成你们的key*/
  ...
  qq.maps.event.addListener(map, "click", function (e) {
    document.getElementById("pointInput").value = e.latLng.getLat().toFixed(6) + "," + e.latLng.getLng().toFixed(6);
    url3 = encodeURI("https://apis.map.qq.com/ws/geocoder/v1/?location=" + e.latLng.getLat() + "," + e.latLng.getLng() + "&key=PLIBZ-P5PWU-RWBVI-2CGW6-LJ6IT-UZFVR&output=jsonp&&callback=?");
  ...
  qq.maps.event.addDomListener(btnSearch, 'click', function () {
    ...
    url = encodeURI("https://apis.map.qq.com/ws/place/v1/search?keyword=" + value + "&boundary=region(" + query_city + ",0)&page_size=9&page_index=1&key=PLIBZ-P5PWU-RWBVI-2CGW6-LJ6IT-UZFVR&output=jsonp&&callback=?");
  ...
  function setFlagClicked(arr, index) {
    each(markerArray, function (n, ele) {
      if (n == index) {
        ...
        document.getElementById("pointInput").value = latLng.getLat().toFixed(6) + "," + latLng.getLng().toFixed(6);
      } else {
        ele.isClicked = false;
  ...
  city.onclick = function (e) {
    ...
    if (target.className == "city_name") {
      ...
      url2 = encodeURI("https://apis.map.qq.com/ws/geocoder/v1/?region=" + curCity.children[0].innerHTML + "&address=" + curCity.children[0].innerHTML + "&key=PLIBZ-P5PWU-RWBVI-2CGW6-LJ6IT-UZFVR&output=jsonp&&callback=?");
  ...
  $(".search_t").autocomplete({
    source:function(request,response){
      url4 = encodeURI("https://apis.map.qq.com/ws/place/v1/suggestion/?keyword=" + request.term + "&region=" + curCity.children[0].innerHTML + "&key=PLIBZ-P5PWU-RWBVI-2CGW6-LJ6IT-UZFVR&output=jsonp&&callback=?");
